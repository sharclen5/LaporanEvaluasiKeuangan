<x-layout :title="$title ?? 'Edit Profile'">
    <x-slot name="header">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg flex justify-between items-start">
                ...
                <div class="flex flex-col items-center mr-48">
                    <!-- Circle Image -->
                    <div
                        class="w-52 h-52 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden mb-8">
                        @if ($user->photo)
                            <img id="image-preview" src="{{ asset('storage/' . $user->photo) }}" alt="{{ $user->name }}"
                                class="w-full h-full object-cover">
                        @else
                            <img id="image-preview" src="" alt="Preview Image" class="hidden w-full h-full object-cover">
                            <span id="no-image-text" class="text-gray-500">No Image</span>
                        @endif
                    </div>
